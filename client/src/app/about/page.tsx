...
const platformPillars = [
  ...
  },
  {
    ...
    description: "Hold funds until the transaction condition is met, then release or refund cleanly.",
  ...
  return (
    <PublicSiteShell currentPage="about">
      <PublicPageHero
        ...
        description="ByReiXwift is an online payment platform being built for users and merchants who want clearer digital payments shaped by Shariah principles. It brings payments, transfers, and escrow-backed protection into one product."
        aside={
          <PublicGlassPanel className="max-w-xl lg:max-w-none">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
              {platformPillars.map((pillar) => (
                <div
                  key={pillar.title}
                  className="rounded-[1.5rem] border border-white/8 bg-white/3 p-4"
                >
      ...
      >
        <div className="flex flex-wrap gap-3">
          ...
          <Link
            href="/contact"
            className="inline-flex items-center gap-2 rounded-full border border-border bg-card/65 px-5 py-3 text-sm font-semibold text-foreground transition-colors hover:border-primary/25 hover:text-primary"
          >
            Contact the team
          </Link>
        </div>
      ...
      <PublicSection className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
        <PublicSectionHeading
          ...
          description="Many digital payment flows still hide too much until the last step. Fees can feel unclear, trust can depend on assumption, and sensitive transactions often need more structure than a direct transfer can provide. ByReiXwift is being built to make those terms easier to review from the start."
        />
        <PublicGlassPanel>
      ...
      <PublicSection className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        <PublicGlassPanel>
          ...
          <div className="mt-6 grid gap-4">
            {audienceCards.map((card) => (
              <div key={card.title} className="rounded-[1.5rem] border border-white/8 bg-white/3 p-5">
        ...
        <div className="rounded-[2.25rem] border border-border bg-card/35 p-8">
          <PublicSectionHeading
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {differentiators.map((item) => (
              <div key={item} className="rounded-[1.5rem] border border-border bg-background/55 p-5">
