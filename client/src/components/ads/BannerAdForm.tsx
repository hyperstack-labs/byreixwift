  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <div
          className={`border-2 border-dashed ${errors.image ? "border-red-500" : "border-(--byreix-border)"} rounded-xl p-4 flex flex-col items-center justify-center bg-(--byreix-bg)/50 hover:bg-(--byreix-bg) transition-colors relative overflow-hidden group min-h-50`}
        >
          {previewUrl ? (
            <div className="relative w-full h-full min-h-50 flex items-center justify-center bg-black/20">
              {mediaType === "video" ? (
                <video src={previewUrl} controls className="max-h-50 w-auto max-w-full rounded" />
              ) : (
            ...
            <label className="cursor-pointer flex flex-col items-center gap-2 w-full h-full py-8">
              ...
              <p className="text-sm text-(--byreix-text-secondary)">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-3 p-3 bg-(--byreix-bg) border border-(--byreix-border) rounded-lg">
            ...
            <span
              className={`text-sm font-bold uppercase ${status === "active" ? "text-(--byreix-green)" : "text-(--byreix-text-secondary)"}`}
            >
              {status}
            </span>
          </div>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          {PLACEMENT_OPTIONS.map((option) => (
            <div
              ...
              className={`cursor-pointer border rounded-lg p-3 text-sm font-medium transition-all ${
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`w-4 h-4 rounded border flex items-center justify-center ${
                    selectedPlacements?.includes(option.id)
                      ? "bg-(--byreix-green) border-(--byreix-green)"
                      : "border-(--byreix-text-secondary)"
                  }`}
                >
                  {selectedPlacements?.includes(option.id) && (
        ...
        {errors.placements && (
          <p className="text-xs text-red-500 mt-1">{errors.placements.message}</p>
        )}
      </div>
