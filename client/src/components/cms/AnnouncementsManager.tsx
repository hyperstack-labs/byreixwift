...
export const AnnouncementsManager = () => {
  ...
  const getTypeStyles = (type: string) => {
    switch (type) {
      ...
      case "promo":
        return "text-(--byreix-green) bg-(--byreix-green)/10 border-(--byreix-green)/20";
      default:
    ...
    try {
      ...
      if (currentEdit) {
        setAnnouncements((prev) =>
          prev.map((a) => (a.id === currentEdit.id ? { ...a, ...payload, id: a.id } : a))
        );
      } else {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-(--byreix-text-secondary) text-sm">
            Manage global banners and notifications.
          </p>
        </div>
        <Button
          onClick={() => {
              ...
              {announcements.length > 0 ? (
                announcements.map((ann) => {
                  ...
                  return (
                    <div
                      key={ann.id}
                      className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-white/2 transition-colors"
                    >
                      <div className="flex gap-4 flex-1 min-w-0">
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${getTypeStyles(ann.type)}`}
                        >
                          <Megaphone size={18} />
                        </div>
                        <div className="min-w-0">
                          <h3 className="text-white font-bold text-sm truncate">{ann.title}</h3>
                          <p className="text-(--byreix-text-secondary) text-xs truncate max-w-md">
                            {ann.message}
                          </p>
                          <div className="flex flex-wrap items-center gap-3 mt-2">
                            <span
                              className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase border ${status.color}`}
                            >
                              {status.label}
                            </span>
          ...
          <div className="bg-(--byreix-surface) border border-(--byreix-border) w-full max-w-lg rounded-2xl p-6 shadow-2xl space-y-6 text-left">
            <div className="flex justify-between items-center border-b border-(--byreix-border) pb-4">
              <h2 className="text-xl font-bold text-white">
                {currentEdit ? "Edit" : "Create"} Announcement
              </h2>
              <button
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-(--byreix-bg) border border-(--byreix-border) rounded-lg p-3 text-sm text-white outline-none focus:border-(--byreix-green) transition-colors cursor-pointer"
                ...
                <div className="col-span-2 space-y-1">
                  ...
                  <select
                    name="type"
                    className="w-full bg-(--byreix-bg) border border-(--byreix-border) rounded-lg p-3 text-sm text-white focus:border-(--byreix-green) transition-colors outline-none cursor-pointer"
                    defaultValue={currentEdit?.type || "info"}
                    title="type-select"
                  >
                ...
                <div className="col-span-2 space-y-1">
                  ...
                  <textarea
                    ...
                    required
                    rows={3}
                    className="w-full bg-(--byreix-bg) border border-(--byreix-border) rounded-lg p-3 text-sm text-white outline-none focus:border-(--byreix-green) transition-colors resize-none "
                    defaultValue={currentEdit?.message}
                    placeholder="Details..."
                  />
                </div>
                ...
                <div className="col-span-2 flex items-center justify-between mt-2">
                  ...
                  <div className="flex gap-2 ">
                    <button
                      ...
                      className="text-[10px] bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-2 py-1 rounded transition-colors cursor-pointer"
                    >
                      Next 7 Days
                    </button>
                    <button
                ...
                <div className="space-y-1 relative">
                  ...
                  <div className="relative group">
                    <Calendar
                      size={14}
                      className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-(--byreix-green) transition-colors pointer-events-none"
                    />
                    <input
                      ref={startRef}
                      name="startDate"
                      ...
                      defaultValue={
                        currentEdit?.startDate || new Date().toISOString().split("T")[0]
                      }
              ...
              <div className="flex gap-3 pt-4 border-t border-(--byreix-border)">
                <Button
                  ...
                  variant="outline"
                  onClick={() => setIsModalOpen(false)}
                  className="flex-1 text-white border-zinc-800 cursor-pointer"
                  disabled={isSubmitting}
                >
                  Cancel
                </Button>
                <Button
                  type="submit"
                  className="flex-1 bg-(--byreix-green) text-(--byreix-bg) font-bold cursor-pointer"
                  disabled={isSubmitting}
                >
                  {isSubmitting ? (
